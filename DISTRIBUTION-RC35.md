# LambLink 1.0.0-rc35 진단 테스트·배포 안내
...
완료 후 `dist\LambLink-v1.0.0-rc35-distribution\` 아래에 두 폴더가 생성됩니다.

- `CDN-UPLOAD\`: 이 안의 네 파일을 CloudFront 원본의 `/releases/` 경로에 같은 이름으로 업로드합니다.
- `USER-DOWNLOAD\`: `LambLink-Setup-1.0.0-rc35.exe` 하나를 사용자에게 배포합니다.
...
```text
%LOCALAPPDATA%\LambLink\companion-rc35.log
```
...
- Companion: `%LOCALAPPDATA%\LambLink\companion-rc35.log` 및 `.1`~`.4`
