...
if (-not $GamePath) {
    $GamePath = Find-CotlPath (Find-SteamRoot)
}
...
if (-not (Test-Path $bepinex)) {
    throw 'BepInEx가 설치되어 있지 않습니다. 현재 RC1 설치기는 기존 BepInEx 설치를 전제로 합니다.'
}
...
if (-not $apiFound) {
    throw 'COTL_API.dll을 찾지 못했습니다. 현재 RC1 설치기는 기존 COTL_API 설치를 전제로 합니다.'
}
...
New-Item -ItemType Directory -Force -Path $pluginDest, $companionDest | Out-Null
...
Write-Host '다음 단계: 게임을 실행한 뒤 Companion을 실행하고 브라우저에서 CHZZK 연결을 승인하세요.'
