...
if (-not (Test-Path (Join-Path $pluginSource 'LambLink.Mod.dll'))) { throw 'Plugin 폴더의 Mod DLL이 없습니다.' }
if (-not (Test-Path (Join-Path $companionSource 'LambLink.Companion.exe'))) { throw 'Companion 실행 파일이 없습니다.' }
...
$pluginDest = Join-Path $bepinex 'plugins\LambLink'
$fontDest = Join-Path $bepinex 'plugins\COTL_KoreanFontFix'
$companionDest = Join-Path $env:LOCALAPPDATA 'Programs\LambLink'
...
if (-not $NoDesktopShortcut) {
    ...
    $shortcutPath = Join-Path $desktop 'LambLink Companion.lnk'
    $shell = New-Object -ComObject WScript.Shell
    $shortcut = $shell.CreateShortcut($shortcutPath)
    $shortcut.TargetPath = Join-Path $companionDest 'LambLink.Companion.exe'
