# ChzzkOfTheLamb v1.0.0 RC32 — diagnostic test GUI Installer

외부 사용자 배포 방식이 `ZIP + PowerShell`에서 **설치 EXE 하나**로 변경되었습니다.

## 사용자 설치 흐름

1. `ChzzkOfTheLamb-Setup-1.0.0-rc32.exe` 실행
2. 설치기가 Steam 라이브러리에서 Cult of the Lamb 자동 탐색
3. 아래 구성요소 다운로드 + SHA-256 검증 + 자동 설치
   - BepInEx 5.4.21 x64
   - COTL_API 0.3.4
   - COTL Korean Font Fix 4.2.1
   - ChzzkOfTheLamb Mod 1.0.0-rc32
   - ChzzkOfTheLamb Companion 1.0.0-rc32 (self-contained)
...
3. 생성된 `CDN-UPLOAD` 폴더의 파일과 `SHA256SUMS.txt` 확인
   ...
   - `release-hosting\installer-manifest-1.0.0-rc32.json` 생성
4. 아래 자체 파일을 CloudFront `/releases/` 경로에 업로드합니다.
   - `COTL-KoreanFontFix-4.2.1-rc32.zip`
   - `ChzzkOfTheLamb-Mod-1.0.0-rc32.zip`
   - `ChzzkOfTheLamb-Companion-1.0.0-rc32-win-x64.zip`
   - `installer-manifest-1.0.0-rc32.json`
5. 실제 후원 진단 테스트를 통과한 뒤 사용자에게 `ChzzkOfTheLamb-Setup-1.0.0-rc32.exe` 하나만 배포합니다.
...
### RC32 verification
Companion must display `v1.0.0-rc32`. After CHZZK login, it must print the `[VIEWER PAGE]`
...
`[BUILD=rc32-donation-overlay-fifo]`,
