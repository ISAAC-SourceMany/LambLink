...
$dist = Join-Path $root 'dist\rc30-companion'
...
foreach ($forbidden in @('[FOLLOWER-MIGRATION][RC26-RESTORED]', 'name drift retained for repair')) {
  ...
  if ($found) { throw "RC30 Companion contains forbidden unsaved-result recovery marker: $forbidden" }
}
foreach ($required in @('[DONATION][TERMINAL][ACK-TIMEOUT]', '[SUPPORT][READY]', 'companion-rc30.log')) {
  ...
  if (-not $found) { throw "RC30 Companion validation assembly is missing diagnostic marker: $required" }
}
Write-Host '[VERIFY] RC30 diagnostic markers found in compiled Companion assembly.'
...
if ($version.FileVersion -ne '1.0.0.30') {
  throw "Unexpected Companion file version: $($version.FileVersion)"
}

Write-Host '[OK] RC30 Companion diagnostics build verified.'
