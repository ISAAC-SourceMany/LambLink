...
$dist = Join-Path $root 'dist\rc34-companion'
...
$criticalSources = @{
  'src\ChzzkOfTheLamb.Companion\Program.cs' = '10e409923e37f2b576d4f6f40fcb9081131111f762610d82d08e37f113571eb2'
  ...
  'src\ChzzkOfTheLamb.Companion\Overlay\RaffleOverlayServer.cs' = '5d1e2fdc3c4ac7ced034e5f6f435d69a9850c28a9f13e04069e263b030e6ded3'
  'src\ChzzkOfTheLamb.Companion\ChzzkOfTheLamb.Companion.csproj' = 'f73a7733eeb9c28cd227c5930bcd4fe7381f68d55405ebc5f9a1a9c8dba807d8'
...
foreach ($relativePath in $criticalSources.Keys) {
  ...
  if (-not (Test-Path $sourcePath)) { throw "Missing critical RC34 source: $relativePath" }
  $actualHash = (Get-FileHash $sourcePath -Algorithm SHA256).Hash.ToLowerInvariant()
  if ($actualHash -ne $criticalSources[$relativePath]) {
    throw "Critical RC34 source does not match the reviewed version: $relativePath"
  }
}
...
foreach ($forbidden in @('[FOLLOWER-MIGRATION][RC26-RESTORED]', 'name drift retained for repair')) {
  ...
  if ($found) { throw "RC34 Companion contains forbidden unsaved-result recovery marker: $forbidden" }
}
foreach ($required in @('RC34_TEST_TOOLS enabled', 'dev donation', '[DONATION][TERMINAL][ACK-TIMEOUT]', '[DONATION][GATE][RX]', 'pausedWhileModGateBlocked=true', '[OVERLAY][BUFF-TIMER][PAUSED]', '[OVERLAY][BUFF-GROUP]', '[OVERLAY][LAYOUT] donation=viewport-top-left-18,width=480px-only,buffs=viewport-left-to-right', '#wrap.donationView{position:fixed!important;left:18px!important', 'width:min(480px,calc(100vw - 36px))!important', '#buffs{position:fixed!important;left:18px!important;right:auto!important', 'direction:ltr', 'justify-content:flex-start!important', 'wrap.getBoundingClientRect().height+10', '[OVERLAY][DONATION-QUEUE][ENQUEUED]', '[OVERLAY][DONATION-QUEUE][DISPLAY]', '[OVERLAY][DONATION-QUEUE][COMPLETED]', 'DONATION_GATE=', '[SUPPORT][READY]', 'companion-rc34.log')) {
  ...
  if (-not $found) { throw "RC34 Companion validation assembly is missing diagnostic marker: $required" }
}
Write-Host '[VERIFY] RC34 diagnostic markers found in compiled Companion assembly.'
...
if ($version.FileVersion -ne '1.0.0.34') {
  throw "Unexpected Companion file version: $($version.FileVersion)"
}

Write-Host '[OK] RC34 Companion diagnostics build verified with RC34_TEST_TOOLS.'
