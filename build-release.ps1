...
Write-Host '[0/9] Verifying RC25 source identity and removing stale compiler outputs...'
$criticalSources = @{
  ...
  'src\ChzzkOfTheLamb.Companion\Program.cs' = 'd197e97ed69820a87bff71a1ed141b3cf9c9b298cca38b59369eabfc2ba01b3f'
  ...
  'src\ChzzkOfTheLamb.Companion\ChzzkOfTheLamb.Companion.csproj' = '947bfd8250c96ed0a75dd8fa4fa2bb5565270c94279159e9d2dedf1f69061b11'
}
...
Write-Host '[VERIFY] Critical RC25 source hashes OK.'
...
$buildTag = 'rc25-cotl-api-runtime-dependency-build-fix'
...
if (-not $hasBuildTag) {
  throw 'Built mod DLL does not contain the RC25 build tag. Refusing to package a stale DLL.'
}
foreach ($marker in @('io.github.xhayper.COTL_API', 'RAFFLE_ROUND_CLOSED')) {
  ...
  if (-not $hasMarker) { throw "Built mod DLL is missing required RC25 marker: $marker" }
}
Write-Host "[VERIFY] RC25 mod build tag found; SHA-256=$((Get-FileHash $modDll -Algorithm SHA256).Hash.ToLowerInvariant())"
...
if ($companionVersion.FileVersion -ne '1.0.0.25') { throw "Unexpected Companion file version: $($companionVersion.FileVersion)" }
Write-Host '[VERIFY] RC25 Companion version tag found.'
