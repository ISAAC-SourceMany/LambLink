# ChzzkOfTheLamb 1.0.0-rc30 진단 테스트·배포 안내
...
`installer-manifest-1.0.0-rc30.json`의 SHA-256을 상호 검증합니다.

## 배포 파일

완료 후 `dist\ChzzkOfTheLamb-v1.0.0-rc30-distribution\` 아래에 두 폴더가 생성됩니다.

- `CDN-UPLOAD\`: 이 안의 네 파일을 CloudFront 원본의 `/releases/` 경로에 같은 이름으로 업로드합니다.
- `USER-DOWNLOAD\`: `ChzzkOfTheLamb-Setup-1.0.0-rc30.exe` 하나를 사용자에게 배포합니다.

설치기는 rc30 전용 매니페스트만 허용합니다. rc30 실후원 검증 전에는 CDN에 업로드하거나
rc29 파일을 교체하지 않습니다. 검증 후 rc30 설치기는 `installer-manifest-1.0.0-rc30.json`과
rc30 구성요소만 참조합니다.
...
```text
%LOCALAPPDATA%\ChzzkOfTheLamb\companion-rc30.log
```
...
`[DONATION][TERMINAL][ACK-TIMEOUT]`은 전송 뒤 15초 안에 게임 결과를 받지 못했다는 뜻입니다.
그 줄의 `gameSocket`, `sync`, `area`, `effect`를 함께 확인합니다.
...
1. CDN의 `installer-manifest-1.0.0-rc30.json`을 열어 `release`가 `1.0.0-rc30`인지 확인합니다.
2. 기존 Companion과 게임을 종료한 클린 PC에서 설치 EXE를 실행합니다.
3. Companion 첫 줄이 `v1.0.0-rc30`인지 확인합니다.
...
6. BepInEx 로그에서 `[BUILD=rc30-support-diagnostics-live-donation]`을 확인합니다.
7. 저장 슬롯·외형 카탈로그 동기화 후 자동 래플과 초록색 `Chzzk` 이름표를 재확인합니다.
8. 최소 금액 실제 후원을 마을과 던전에서 각각 1회 테스트하고 요청 ID 전 단계를 확인합니다.
9. 실패하거나 단계가 누락되면 즉시 `support`를 실행해 생성된 ZIP을 보관합니다.
10. `SHA256SUMS.txt`는 빌드 산출물과 업로드 파일 대조용으로 보관합니다.

## 로그

- 설치기: `%LOCALAPPDATA%\ChzzkOfTheLamb\installer.log`
- Companion: `%LOCALAPPDATA%\ChzzkOfTheLamb\companion-rc30.log` 및 `.1`~`.4`
