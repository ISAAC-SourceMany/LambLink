...
$fontPath = Join-Path $outDir 'COTL-KoreanFontFix-4.2.1.zip'
$modPath = Join-Path $outDir 'ChzzkOfTheLamb-Mod-1.0.0.zip'
$companionPath = Join-Path $outDir 'ChzzkOfTheLamb-Companion-1.0.0-win-x64.zip'
...
$outManifest = Join-Path $outDir 'installer-manifest.json'
Set-Content $outManifest $json -Encoding UTF8
Write-Host "[OK] Installer manifest: $outManifest"
Write-Host '[NEXT] Upload your own component ZIPs + installer-manifest.json under CloudFront /releases/.'
