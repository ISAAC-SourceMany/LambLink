...
internal sealed class SupportBundleService(string dataDirectory, string releaseVersion, string companionLogPath)
{
    ...
    public SupportBundleResult Create(SupportBundleSnapshot snapshot)
    {
        ...
        try
        {
            ...
            var zipPath = Path.Combine(destinationDirectory,
                $"ChzzkOfTheLamb-Support-{createdAt:yyyyMMdd-HHmmss}Z-{reportId}.zip");
    ...
    private static IReadOnlyList<object> CollectRuntimeHashes(string? gameRoot, string? companionRoot)
    {
        ...
        if (!string.IsNullOrWhiteSpace(gameRoot))
        {
            ...
            candidates.Add(("ChzzkOfTheLamb.Mod.dll", Path.Combine(plugins, "ChzzkOfTheLamb", "ChzzkOfTheLamb.Mod.dll")));
            candidates.Add(("ChzzkOfTheLamb.Protocol.dll", Path.Combine(plugins, "ChzzkOfTheLamb", "ChzzkOfTheLamb.Protocol.dll")));
        ...
        if (!string.IsNullOrWhiteSpace(companionRoot))
            candidates.Add(("ChzzkOfTheLamb.Companion.exe", Path.Combine(companionRoot, "ChzzkOfTheLamb.Companion.exe")));
