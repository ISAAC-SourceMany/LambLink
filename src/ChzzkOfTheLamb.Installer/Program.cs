...
namespace ChzzkOfTheLamb.Installer;
...
{
    private const string ReleaseVersion = "1.0.0-rc29";
    private const string DefaultManifestUrl = "https://d1gvw9ccym1qvn.cloudfront.net/releases/installer-manifest-1.0.0-rc29.json";
    ...
    {
        Text = "ChzzkOfTheLamb Setup 1.0.0-rc29";
        Width = 720;
        Height = 500;
        ...
        var root = new TableLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(16), RowCount = 7, ColumnCount = 1 };
        ...
        var title = new Label { Text = "ChzzkOfTheLamb", Font = new Font(Font.FontFamily, 18, FontStyle.Bold), AutoSize = true };
        ...
        var components = new Label { Text = "설치 구성: BepInEx 5.4.21 · COTL_API 0.3.4 · Korean Font Fix 4.2.1 · ChzzkOfTheLamb Mod · Companion", AutoSize = true };
    ...
    private async Task InstallAsync()
    {
        ...
        var workDir = Path.Combine(Path.GetTempPath(), "ChzzkOfTheLamb-Setup", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            Log($"[MANIFEST] release={manifest.Release}, components={manifest.Components.Count}, source={manifestUrl}");

            var gameRoot = _gamePath.Text;
            var companionRoot = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "ChzzkOfTheLamb");
            ...
            _companionExe = Path.Combine(companionRoot, "ChzzkOfTheLamb.Companion.exe");
            CreateShortcuts(_companionExe);
            WriteInstallReceipt(manifest, gameRoot, companionRoot);
            _status.Text = "설치가 완료되었습니다.";
            SetProgress(100);
            _launch.Enabled = File.Exists(_companionExe);
            Log("[COMPLETE] installation successful");
            MessageBox.Show(this, "설치가 완료되었습니다.\nCompanion을 실행한 뒤 CHZZK 로그인을 진행하세요.", "설치 완료", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    ...
    private static async Task<InstallerManifest> DownloadManifestAsync(string url, CancellationToken token)
    {
        ...
        http.DefaultRequestHeaders.UserAgent.ParseAdd("ChzzkOfTheLamb-Installer/1.0.0-rc29");
    ...
    private void CreateShortcuts(string exe)
    {
        ...
        try
        {
            ...
            var startMenu = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "ChzzkOfTheLamb");
            Directory.CreateDirectory(startMenu);
            CreateUrlShortcut(Path.Combine(desktop, "ChzzkOfTheLamb Companion.url"), exe);
            CreateUrlShortcut(Path.Combine(startMenu, "ChzzkOfTheLamb Companion.url"), exe);
            Log("[SHORTCUT] desktop/start-menu shortcuts created");
        }
    ...
    {
        var stateDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ChzzkOfTheLamb");
        Directory.CreateDirectory(stateDir);
        var receipt = new { installedAtUtc = DateTime.UtcNow, manifest.Release, gameRoot, companionRoot, manifest.Components };
...
internal static class InstallerLog
{
    ...
    public static readonly string FilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ChzzkOfTheLamb", "installer.log");
