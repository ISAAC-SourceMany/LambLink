...
using LambLink.Companion.Appearance;
using LambLink.Companion.Chzzk;
using LambLink.Companion.Cloud;
using LambLink.Companion.Configuration;
using LambLink.Companion.Diagnostics;
using LambLink.Companion.GameBridge;
using LambLink.Companion.Raffle;
...
var launchProfile = IsReleaseDistribution
    ? CompanionLaunchProfileLoader.Load(executableDirectory, ReleaseVersion)
    : null;
var stagingEnvironmentRequested = IsTruthy(Environment.GetEnvironmentVariable("COTL_STAGING_MODE"));
var stagingMode = IsReleaseDistribution
                  && (stagingEnvironmentRequested || launchProfile?.IsStaging == true);
var defaultDataDirectoryName = stagingMode ? "LambLink-Staging" : "LambLink";
var legacyDataDirectoryName = stagingMode ? "ChzzkOfTheLamb-Staging" : "ChzzkOfTheLamb";
var stagingDataDirOverride = stagingMode
    ? Environment.GetEnvironmentVariable("COTL_STAGING_DATA_DIR") ?? launchProfile?.DataDirectory
    : null;
...
var migratedLegacyFileCount = 0;
string? legacyMigrationWarning = null;
try
{
    migratedLegacyFileCount = LegacyDataMigration.CopyMissingFiles(legacyDataDir, dataDir);
}
catch (Exception ex)
{
...
Console.WriteLine($"[ENVIRONMENT] selected={(stagingMode ? "staging" : "production")}, source={(stagingEnvironmentRequested ? "process-environment" : launchProfile is not null ? "installed-launch-profile" : "release-default")}, executable={Environment.ProcessPath}, data={dataDir}");
if (migratedLegacyFileCount > 0)
    Console.WriteLine($"[BRAND-MIGRATION] copied={migratedLegacyFileCount}, source={legacyDataDir}, target={dataDir}, overwrite=false");
if (!string.IsNullOrWhiteSpace(legacyMigrationWarning))
    Console.Error.WriteLine($"[BRAND-MIGRATION][WARN] source={legacyDataDir}, target={dataDir}, error={legacyMigrationWarning}");
...
Console.WriteLine($"LambLink for Cult of the Lamb - v{ReleaseVersion}");
if (IsReleaseDistribution)
{
    if (stagingMode)
    {
        try { Console.Title = $"LambLink Companion {ReleaseVersion} — STAGING TEST"; } catch { }
        var previousColor = Console.ForegroundColor;
        try
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine("============================================================");
            Console.WriteLine("[STAGING TEST] 운영 환경이 아닙니다. 데이터와 시청자 웹이 분리됩니다.");
            Console.WriteLine("============================================================");
        }
        finally
        {
...
var configuredWebApiBase = IsReleaseDistribution
    ? stagingMode
        ? RequireHttpsValue("COTL_WEB_API_BASE", Environment.GetEnvironmentVariable("COTL_WEB_API_BASE") ?? launchProfile?.ApiBaseUrl)
        : ProductionApiBase
...
var frontendUrl = IsReleaseDistribution
    ? stagingMode
        ? RequireHttpsValue("COTL_WEB_FRONTEND_URL", Environment.GetEnvironmentVariable("COTL_WEB_FRONTEND_URL") ?? launchProfile?.FrontendUrl)
...
var followerRosterReconcileGate = new SemaphoreSlim(1, 1);
var hydratedFollowerStateSaves = new HashSet<string>(StringComparer.Ordinal);
var followerStateDeferralLogged = new HashSet<string>(StringComparer.Ordinal);
...
bridge.ConnectionChanged += connected =>
{
    ...
    if (connected)
    {
        lastNameplateSyncSignature = string.Empty;
    ...
    try
    {
        switch (envelope.Type)
        {
            ...
            case GameMessageTypes.FollowerRoster:
            {
                ...
                _ = ReconcileFollowerRosterAsync(roster);
                break;
            }
...
var rosterRefreshInterval = TimeSpan.FromSeconds(30);
Console.WriteLine($"[FOLLOWER-ROSTER] event-driven lifecycle enabled; fallback snapshot every {rosterRefreshInterval.TotalSeconds:0}s");
    ...
    while (!stop.IsCancellationRequested)
    {
        try
        {
            if (bridge.IsGameConnected && lastGameStatus?.InGame == true && currentSaveId != "unknown")
                await bridge.SendAsync(GameMessageTypes.GetFollowerRoster, new { }, stop.Token);
            await Task.Delay(rosterRefreshInterval, stop.Token);
        }
        catch (OperationCanceledException) when (stop.IsCancellationRequested) { break; }
        catch (Exception ex)
        {
            Console.WriteLine($"[FOLLOWER-ROSTER] refresh warning: {ex.Message}");
            try { await Task.Delay(rosterRefreshInterval, stop.Token); } catch { break; }
        }
    }
}, stop.Token);
...
async Task ReconcileFollowerRosterAsync(FollowerRosterSnapshot roster)
{
    if (string.IsNullOrWhiteSpace(roster.SaveId) || roster.SaveId == "unknown") return;

    try
    {
        await followerRosterReconcileGate.WaitAsync(stop.Token);
    }
    ...
    try
    {
        var canUseFollowerState = await EnsureFollowerStateHydratedAsync(roster.SaveId);
        var localRecords = followers.GetForSave(streamerChannelId, roster.SaveId);
        if (!canUseFollowerState && localRecords.Count == 0)
        {
            if (followerStateDeferralLogged.Add(roster.SaveId))
                Console.WriteLine($"[FOLLOWER-STATE][DEFERRED] save={roster.SaveId}; cloud recovery is not ready and no local mappings exist. Empty marker sync is suppressed.");
            return;
        }

        // A newer roster can arrive while cloud hydration is in flight. Let the queued newer
        // reconciliation own lifecycle and marker decisions instead of applying stale input.
        if (string.Equals(latestRosterSaveId, roster.SaveId, StringComparison.Ordinal) && latestRosterAt > roster.GeneratedAt)
            return;

        followerStateDeferralLogged.Remove(roster.SaveId);
        var rosterFollowersById = roster.Followers
            .Where(x => x.FollowerId > 0)
            .GroupBy(x => x.FollowerId)
            .ToDictionary(g => g.Key, g => g.First());
        ...
        {
            // Missing roster entries are not proof of deletion: followers may be dead,
            // imprisoned, away on an expedition, or between vanilla lifecycle collections.
            if (!rosterFollowersById.TryGetValue(record.FollowerId, out var actual)) continue;

            if (!FollowerRosterNameMatchesViewer(actual.Name, record.FollowerName))
            {
                // Follower IDs are reusable and an unsaved raffle result legitimately disappears
                // after exit. Never decorate or rename a different follower from ID alone.
                stale.Add(record);
            ...
            {
                if (followers.ApplyLifecycle(streamerChannelId, $"roster-died:{roster.SaveId}:{record.FollowerId}:{roster.GeneratedAt.ToUnixTimeSeconds()}", roster.SaveId, record.FollowerId, "Died", roster.GeneratedAt, actual.DeathReason ?? "Unknown"))
            ...
            {
                if (followers.ApplyLifecycle(streamerChannelId, $"roster-resurrected:{roster.SaveId}:{record.FollowerId}:{roster.GeneratedAt.ToUnixTimeSeconds()}", roster.SaveId, record.FollowerId, "Resurrected", roster.GeneratedAt, null))
                    _ = SyncViewerFollowerStateAsync(record.ViewerChannelId, roster.SaveId);
            }
        }
        ...
        var rosterFingerprint = roster.SaveId + ":" + string.Join(",", rosterFollowersById.Keys.OrderBy(x => x));
        if (stale.Count > 0 || !string.Equals(lastRosterFingerprint, rosterFingerprint, StringComparison.Ordinal))
        {
            Console.WriteLine($"[FOLLOWER-ROSTER] save={roster.SaveId}, actual={rosterFollowersById.Count}, staleRemoved={stale.Count}, ids=[{string.Join(",", rosterFollowersById.Keys.OrderBy(x => x))}]");
        ...
        await RecoverPendingReservationsAsync(roster);
        await SyncChzzkMarkersToGameAsync(roster.SaveId, validateAgainstRoster: true);
    }
    catch (OperationCanceledException) when (stop.IsCancellationRequested) { }
    catch (Exception ex)
    {
        Console.WriteLine($"[FOLLOWER-RECONCILE] roster processing failed for save={roster.SaveId}: {ex.Message}");
    }
    finally
    {
        followerRosterReconcileGate.Release();
    }
}

async Task<bool> EnsureFollowerStateHydratedAsync(string saveId)
{
    if (hydratedFollowerStateSaves.Contains(saveId)) return true;

    var localCountBefore = followers.GetForSave(streamerChannelId, saveId).Count;
    if (developmentMode || string.IsNullOrWhiteSpace(cloudBaseUrl))
        return true;
    ...
    try
    {
        if (cloud?.IsAuthenticated != true && !await TryConnectCloudAsync(logFailure: false))
        {
            ...
            {
                Console.WriteLine($"[FOLLOWER-STATE] cloud recovery unavailable for save={saveId}; using {localCountBefore} local record(s).");
                return true;
            }
            return false;
        }

        var states = await cloud!.GetFollowerStatesAsync(streamerChannelId, saveId, stop.Token);
        var replacedStates = 0;
        foreach (var state in states)
        {
            if (!string.Equals(state.SaveId, saveId, StringComparison.Ordinal) || string.IsNullOrWhiteSpace(state.ViewerChannelId))
                continue;

            var recovered = (state.History ?? new List<CloudFollowerHistoryEntry>())
                .Where(x => x.FollowerId > 0 && !string.IsNullOrWhiteSpace(x.FollowerName))
                .Select((x, index) => new ViewerFollowerRecord
                {
                    StreamerChannelId = streamerChannelId,
                    ViewerChannelId = state.ViewerChannelId,
                    LastKnownNickname = string.IsNullOrWhiteSpace(x.ViewerNickname) ? x.FollowerName : x.ViewerNickname,
                    FollowerName = x.FollowerName,
                    SaveId = saveId,
                    FollowerId = x.FollowerId,
                    Generation = Math.Max(1, x.Generation),
                    IsAlive = x.IsAlive,
                    CreatedAt = x.CreatedAt == default ? DateTimeOffset.UnixEpoch : x.CreatedAt,
                    DiedAt = x.DiedAt,
                    DeathReason = x.DeathReason,
                    ResurrectedAt = x.ResurrectedAt,
                    Appearance = x.Appearance,
                    Revision = state.Revision,
                    Events = (x.Events ?? new List<CloudFollowerHistoryEvent>())
                        .Where(e => !string.IsNullOrWhiteSpace(e.Type))
                        .Select((e, eventIndex) => new ViewerFollowerHistoryEvent
                        {
                            EventId = $"cloud:{Math.Max(1, x.Generation)}:{index}:{eventIndex}:{e.Type}",
                            Type = e.Type,
                            At = e.At == default ? (x.CreatedAt == default ? DateTimeOffset.UnixEpoch : x.CreatedAt) : e.At,
                            Cause = e.Cause
                        }).ToList()
                }).ToList();
            if (recovered.Count == 0) continue;
            if (followers.ReplaceStateIfNewer(streamerChannelId, state.ViewerChannelId, saveId, state.Revision, recovered))
                replacedStates++;
        }

        var localCountAfter = followers.GetForSave(streamerChannelId, saveId).Count;
        if (states.Count > 0 && localCountAfter == 0)
        {
            Console.WriteLine($"[FOLLOWER-STATE][INVALID] cloud returned {states.Count} state document(s) for save={saveId}, but none contained a usable follower identity. Empty marker sync is suppressed.");
            return false;
        }

        hydratedFollowerStateSaves.Add(saveId);
        Console.WriteLine($"[FOLLOWER-STATE][HYDRATED] save={saveId}, cloudStates={states.Count}, replacedStates={replacedStates}, localRecords={localCountAfter}");
        return true;
    }
    ...
    catch (Exception ex)
    {
        if (localCountBefore > 0)
        {
            Console.WriteLine($"[FOLLOWER-STATE] cloud recovery failed for save={saveId}; using {localCountBefore} local record(s): {ex.Message}");
            return true;
        }
        Console.WriteLine($"[FOLLOWER-STATE][DEFERRED] cloud recovery failed for save={saveId}; empty marker sync remains suppressed: {ex.Message}");
        return false;
    }
}
...
static string RequireHttpsValue(string name, string? configuredValue)
{
