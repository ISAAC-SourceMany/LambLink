...
public sealed class FollowerAppearanceService(ManualLogSource log, GameSaveService saves, Action<string>? diagnosticStage = null)
{
    ...
    private FollowerAppearanceCatalog? _catalogCache;
    ...
    public FollowerAppearanceCatalog BuildCatalog(bool includeModded, bool includeSpecial)
    {
        ...
        void Detail(string name, string details = "")
        {
            diagnosticStage?.Invoke($"CATALOG/{requestId}/{name}");
            log.LogDebug($"[APPEARANCE][BUILD][{name}] id={requestId}{(string.IsNullOrEmpty(details) ? string.Empty : ", " + details)}");
        }
        ...
        try
        {
            ...
            var cacheKey = BuildCatalogCacheKey(catalog.SaveId, includeModded, includeSpecial, instance, raw.Count, unlocked);
            if (_catalogCache != null && string.Equals(_catalogCacheKey, cacheKey, StringComparison.Ordinal))
            {
                Stage("CACHE-HIT", $"save={catalog.SaveId}, forms={_catalogCache.Forms.Count}");
                Stage("END", $"save={catalog.SaveId}, forms={_catalogCache.Forms.Count}, elapsedMs={ElapsedMilliseconds(started):F1}, cached=true");
            ...
            if (unlocked.Count > 0)
            {
                ...
                for (var formIndex = 0; formIndex < orderedUnlocked.Count; formIndex++)
                {
                    ...
                    Detail("FORM-BEGIN", $"index={formIndex + 1}/{orderedUnlocked.Count}, form={formId}");
                    object? item = null;
                    try
                    {
                        if (getIndex != null)
                    ...
                    {
                        missing++;
                        Detail("FORM-END", $"index={formIndex + 1}/{orderedUnlocked.Count}, form={formId}, result=missing");
                        continue;
                    }
                    ...
                    {
                        Detail("FORM-END", $"index={formIndex + 1}/{orderedUnlocked.Count}, form={formId}, result=duplicate");
                        continue;
                    }
                    if (!includeSpecial && form.IsSpecial)
                    {
                        Detail("FORM-END", $"index={formIndex + 1}/{orderedUnlocked.Count}, form={formId}, result=special-filtered");
                        continue;
                    }
                    if (!includeModded && form.IsModded)
                    {
                        Detail("FORM-END", $"index={formIndex + 1}/{orderedUnlocked.Count}, form={formId}, result=modded-filtered");
                        continue;
                    }
                    catalog.Forms.Add(form);
                    mapped++;
                    Detail("FORM-END", $"index={formIndex + 1}/{orderedUnlocked.Count}, form={formId}, variants={form.VariantIds.Count}, colors={form.ColorIds.Count}");
                }
            }
            else
            {
    ...
    private static string BuildCatalogCacheKey(
        ...
        bool includeModded,
        bool includeSpecial,
        object worshipperData,
        int rawCount,
        IEnumerable<string> unlocked)
    {
        return string.Join("|",
            saveId,
            includeModded ? "modded" : "vanilla",
            includeSpecial ? "special" : "normal",
            RuntimeHelpers.GetHashCode(worshipperData).ToString(System.Globalization.CultureInfo.InvariantCulture),
            rawCount.ToString(System.Globalization.CultureInfo.InvariantCulture),
            string.Join("\u001f", unlocked.OrderBy(x => x, StringComparer.OrdinalIgnoreCase)));
    }
    ...
    public bool CanApply(object target, FollowerAppearanceSelection? selection)
    {
        if (selection is null) return true;
        return TryResolveSelection(target, selection, out _);
    }

    public bool MatchesAppliedSelection(object target, FollowerAppearanceSelection? selection)
    {
        if (selection is null) return true;
        var applied = ReadAppliedSelection(target, null);
        if (applied == null || !string.Equals(applied.FormId, selection.FormId, StringComparison.OrdinalIgnoreCase)) return false;
        if (selection.VariantId != null && ParseIndex(applied.VariantId, -1) != ParseIndex(selection.VariantId, -2)) return false;
        if (selection.ColorId != null && ParseIndex(applied.ColorId, -1) != ParseIndex(selection.ColorId, -2)) return false;
        return true;
    }
    ...
    public bool TryApply(object target, FollowerAppearanceSelection? selection)
    {
        ...
        try
        {
            if (!TryResolveSelection(target, selection, out var resolved)) return false;
            var ok = TryWriteConvertibleValue(resolved.Info, new[] { "SkinCharacter" }, resolved.CharacterIndex.ToString(System.Globalization.CultureInfo.InvariantCulture));
            ok &= TryWriteConvertibleValue(resolved.Info, new[] { "SkinVariation", "SkinVariant", "Variant", "VariantId" }, resolved.VariantIndex.ToString(System.Globalization.CultureInfo.InvariantCulture));
            if (selection.ColorId is not null)
                ok &= TryWriteConvertibleValue(resolved.Info, new[] { "SkinColour", "SkinColor", "Colour", "Color" }, resolved.ColorIndex.ToString(System.Globalization.CultureInfo.InvariantCulture));
            ok &= TryWriteConvertibleValue(resolved.Info, new[] { "SkinName", "Form", "FormId" }, resolved.ActualSkinName);
            return ok;
        }
        catch (Exception ex)
        {
    ...
    private bool TryResolveSelection(object target, FollowerAppearanceSelection selection, out ResolvedAppearance resolved)
    {
        resolved = null!;
        var info = FindFollowerInfo(target, 3);
        if (info is null || string.IsNullOrWhiteSpace(selection.FormId)) return false;

        var unlocked = ReadStringSetFromDataManager("FollowerSkinsUnlocked", "FollowerFormsUnlocked", "SkinsUnlocked");
        if (unlocked.Count > 0 && !unlocked.Contains(selection.FormId)) return false;
        ...
        resolved = new ResolvedAppearance(info, idx, variant, colour, actualSkinName!);
        return true;
    }

    private sealed class ResolvedAppearance(object info, int characterIndex, int variantIndex, int colorIndex, string actualSkinName)
    {
        ...
        public string ActualSkinName { get; } = actualSkinName;
    }
