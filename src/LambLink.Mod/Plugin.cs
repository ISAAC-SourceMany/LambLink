...
public sealed class Plugin : BaseUnityPlugin
{
    ...
    // Preserve the original BepInEx identity so an in-place upgrade cannot load the
    ...
    public const string BuildTag = "v1.0.2-production";
    ...
    private void Awake()
    {
        ...
        try
        {
            var receiptPath = Path.Combine(Paths.ConfigPath, "LambLink", "donation-receipts.json");
            MigrateLegacyDonationReceipts(receiptPath);
    ...
    private void MigrateLegacyDonationReceipts(string receiptPath)
    {
        var legacyPath = Path.Combine(Paths.ConfigPath, "ChzzkOfTheLamb", "donation-receipts.json");
        if (File.Exists(receiptPath) || !File.Exists(legacyPath))
            return;

        Directory.CreateDirectory(Path.GetDirectoryName(receiptPath)!);
        File.Copy(legacyPath, receiptPath, overwrite: false);
        Logger.LogInfo($"[BRAND-MIGRATION] donation receipts copied to {receiptPath}; overwrite=false");
    }
